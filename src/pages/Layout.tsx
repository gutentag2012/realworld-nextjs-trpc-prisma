...
export const Layout: FunctionComponent<Props> = ({ children, privateRoute, restrictForUsername, title }) => {
          ...
          <div className="container">
            ...
            <ul className="nav navbar-nav pull-xs-right">
              ...
              <li className="nav-item">
                <Link
                  className={
                    'nav-link' + (pathname === `/profile/${encodeURIComponent(user.username)}` ? ' active' : '')
                  }
                  href={`/profile/${encodeURIComponent(user.username)}`}
                >
                  {user.image && (
                    <Image src={user.image} alt="Profile picture" className="user-pic" width={26} height={26} />
                  )}
                  {user.username}
                </Link>
              </li>
            ...
            <ul className="nav navbar-nav pull-xs-right">
              {guestLinks.map(([path, text]) => (
                <li key={path} className="nav-item">
                  <Link className={['nav-link', path === pathname && 'active'].filter(Boolean).join(' ')} href={path}>
                    {text}
                  </Link>
                </li>
              ))}
